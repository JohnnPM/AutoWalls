 ...
 * File: TpPlayersCommand.java <br>
 * Package: com.jkush321.autowalls.commands <p>
 * 
 * @author 598Johnn897
 */
public class TpPlayersCommand implements CommandListener {

	private AutoWalls plugin = AutoWalls.get();

	@Command(command = "tpplayers", permission = "walls.tpplayers")
	public void tpSpecs(CommandArgs info) {
		if (info.isPlayer()) {
			Player player = info.getPlayer();
			if (info.getArgs().length == 0) {
				for (Player p : plugin.getHandler().playing) {
			...
			if (info.getArgs().length == 0) {
				...
				Location loc = new Location(plugin.getHandler().playing.get(
						0).getWorld(), x, y, z);

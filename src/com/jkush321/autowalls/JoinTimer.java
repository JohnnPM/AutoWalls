		...
		while (timeleft > 0) {
			try {
				...
				if (timeleft >= 10 && timeleft % 10 == 0) {
					Bukkit.broadcastMessage(ChatColor.DARK_AQUA + "You can "
							+ ChatColor.YELLOW + "/join" + ChatColor.DARK_AQUA
							+ " in " + timeleft + " seconds!"
							+ ChatColor.DARK_RED
							+ " Make sure to use /kit !!!!");
					Bukkit.broadcastMessage(ChatColor.DARK_GREEN + "You can "
							+ ChatColor.YELLOW + "/join "
							+ ChatColor.DARK_GREEN + "at any time if you have "
							+ AutoWalls.earlyJoinPriority
							+ " or more priority!");
				}
				if (timeleft == 5) {
					Bukkit.broadcastMessage(ChatColor.DARK_AQUA + "You can "
				...
				if (timeleft == 0) {
					Bukkit.broadcastMessage(ChatColor.DARK_AQUA
							+ "You can now " + ChatColor.YELLOW + "/join"
							+ ChatColor.DARK_AQUA + " !");
				}
			} catch (Exception e) {
				System.out.println("JoinTimer error.");
				e.printStackTrace();
			}
		}
